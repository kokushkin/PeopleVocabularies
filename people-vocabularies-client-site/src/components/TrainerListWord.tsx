...
const TrainerListWord = ({
  word,
  onKnown,
  onUnknown,
}: {
  word: IWordDescription;
  onKnown: () => void;
  onUnknown: () => void;
}) => {
      ...
      <div className="container">
        <div className="row">
          <div className="container-fluid">
            ...
            {/* KNOWN word button */}
            <button className="btn  float-right" onClick={() => onKnown()}>
            ...
            {/* UNKNOWN word button */}
            <button className="btn  float-right" onClick={() => onUnknown()}>
              <i className="fa fa-2x fa-check text-warning" />
            </button>
          </div>
        </div>
        {showWordTranslations && word.exclusionForms && (
          <div className="row">{word.exclusionForms.join(", ")}</div>
        )}
        <div className="row">
